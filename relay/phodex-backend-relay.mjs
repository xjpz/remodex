// FILE: phodex-backend-relay.mjs
// Purpose: Compact ESM relay used by the Phodex VPS custom Next server.
// Deploy target: /app/codex-backend/src/relay.mjs
...
export function setupRelay(wss) {
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (role === "mac") {
      ...
      console.log(
        `[relay] Mobile connected (${role}) -> session ${sessionId} (${session.clients.size} client(s))`
      );
    ...
    ws.on("close", () => {
      if (role === "mac") {
        ...
        console.log(
          `[relay] Mobile disconnected (${role}) -> session ${sessionId} (${session.clients.size} remaining)`
        );
      }
